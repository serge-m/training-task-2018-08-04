...
class resnetV1_redo_w_smaller_intro_kernel(pl.LightningModule):

    def __init__(self, hparams: Params):
        super().__init__()

        self.hparams = hparams
        self.model = ResNet(BasicBlock, [2, 2, 1, 1], num_classes=10)
...
def main(hparams):
    model = resnetV1_redo_w_smaller_intro_kernel(hparams)
    ...
    print("running with epochs {}".format(hparams.epochs))
    trainer = pl.Trainer(
...
if __name__ == '__main__':
    ...
    parser = resnetV1_redo_w_smaller_intro_kernel.add_model_specific_args(parent_parser)
